...
- [x] **Widget / lock-screen glance of the next slot's weather** (iOS). A
      WidgetKit extension in `targets/widget/`, generated by
      `@bacons/apple-targets`, showing the next stop and its umbrella verdict on
      the home and lock screens. `buildWidgetSnapshot` derives the glance and
      `writeWidgetSnapshot` publishes it into a shared App Group at the end of
      `runNotificationSync` — see [round 29](NOTES.md#round-29--the-widget-the-one-thing-appjson-couldnt-reach)
      and the widget traps in
      [read this first](NOTES.md#read-this-before-writing-code-here). **Two
      manual steps remain before the native build signs**: set `ios.appleTeamId`
      in `app.json`, and register the App Group against both `com.sg.brelly.app`
      and `com.sg.brelly.app.widget` in the Apple Developer account. The Android
      `AppWidgetProvider` from the original backlog is deliberately deferred —
      Android has no lock-screen glance — and the JS seam would feed one
      unchanged if it is ever added.
