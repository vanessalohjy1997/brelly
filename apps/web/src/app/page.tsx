        ...
        actions={
          <>
            {/* Pull-to-refresh has no honest web equivalent — a page that
                ...
                pull-to-reload — so the gesture becomes a control. */}
            <Button tone="quiet" onClick={() => void refresh()} disabled={isRefreshing}>
            ...
            {sortSlotsByStart(todaysPlan.slots).map((slot) => (
              <li key={slot.id}>
                <ItineraryCard
                  ...
                  onToggleMute={() => void toggleMuteWithUndo(todaysPlan.date, slot)}
                />
              </li>
            ))}
          </ul>
