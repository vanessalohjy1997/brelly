"use client";

import { useRouter, useSearchParams } from "next/navigation";
import { useState } from "react";

import {
  describeRoutine,
  saveWithFeedback,
  toDateKey,
  toTimeOfDay,
  useItineraryStore,
  useRoutineStore,
} from "@brelly/core";

import { FormPageHeader } from "@/components/FormPageHeader";
import { SlotForm } from "@/components/itinerary/SlotForm";
import { useRoutineMaterializer } from "@/hooks/useRoutineMaterializer";
import { useUnsavedChangesGuard } from "@/hooks/useUnsavedChangesGuard";

export default function NewPlanPage() {
  const router = useRouter();
  // `?date=` rather than a route param: the day is an *option* on the add form,
  // not part of what the page is, and a link to it without one is still a
  // perfectly good link.
  const date = useSearchParams().get("date") ?? undefined;
  const addSlot = useItineraryStore((state) => state.addSlot);
  const addRoutine = useRoutineStore((state) => state.addRoutine);
  const materializeRoutines = useRoutineMaterializer();
  const [dirty, setDirty] = useState(false);
  const confirmDiscard = useUnsavedChangesGuard(dirty);

  return (
    <>
      <FormPageHeader title="Add plan" confirmDiscard={confirmDiscard} />
      <SlotForm
        submitLabel="Add plan"
        initialDate={date}
        onDirtyChange={setDirty}
        allowRepeat
        onSubmit={({ repeat, ...values }) => {
          const start = new Date(values.startTime);
          const planDate = toDateKey(start);

          // A repeat is stored as a rule and filled in a fortnight at a time
          // rather than written out here — see `planRoutineMaterialization`.
          // The materialiser covers the first day too, so there is no `addSlot`
          // on that branch and no risk of writing that day twice.
          const saved = saveWithFeedback(
            () => {
              if (repeat) {
                addRoutine({
                  label: values.label,
                  location: values.location,
                  latitude: values.latitude,
                  longitude: values.longitude,
                  countryCode: values.countryCode,
                  frequency: repeat.frequency,
                  weekdays: repeat.weekdays,
                  dayOfMonth: repeat.dayOfMonth,
                  startTime: toTimeOfDay(start),
                  endTime: toTimeOfDay(new Date(values.endTime)),
                  startDate: planDate,
                  endDate: repeat.endDate,
                  kind: values.kind,
                  notificationsMuted: values.notificationsMuted,
                });
                return null;
              }
              // The day the form ended up on, not the one this page was opened
              // from — changing the start date in the form has to land the plan
              // on that day.
              return { date: planDate, slot: addSlot(planDate, values) };
            },
            {
              // Names the rule, not a count: "Added Office · Repeats Mon–Fri"
              // is the promise being made, and a number would be a lie about a
              // routine that has no end.
              success: repeat
                ? `Added ${values.label} · ${describeRoutine(repeat)}`
                : `Added ${values.label}`,
              failure: "Couldn't save that plan. Try again.",
            },
          );
          // Staying on the form is the point of the guard: leaving on a failed
          // save would drop everything the user typed.
          if (!saved.ok) return;

          // Fills in the next fortnight straight away, so Plans shows the
          // routine on the way back rather than after a reload.
          if (!saved.value) materializeRoutines();

          // Cleared before navigating, or the guard would ask about a form that
          // has just been saved.
          setDirty(false);
          router.push("/plans");
        }}
      />
    </>
  );
}
